...
sub NewTemplate
{
  ...
  print "<TR><TD>Generate MAC Based PXE config</TD><TD><INPUT TYPE=CHECKBOX NAME=GENERATEMAC CHECKED></TD></TR>\n";
...
sub WriteTemplatePXEMenu
{
  ...
  if ($#sublist > 0)
  {
   ...
   for $cursubindex (sort(keys(%subsort)))
   {
     ...
     if ($item ne "__HEADER__")
     {
       ...
       if (defined($subinfo{MAC}) && $subinfo{MAC} ne "" && $subinfo{GENERATEMAC} ne "OFF")
       {
         my($newmac)=$subinfo{MAC};
         $newmac=~s/:/-/g;
  ...
  if (defined($info{MAC}) && $info{MAC} ne "" && $info{GENERATEMAC} ne "OFF" )
  {
    my($newmac)=$info{MAC};
    $newmac=~s/:/-/g;
