...
sub vsim9_NewTemplate_2
{
 ...
 print "<INPUT TYPE=HIDDEN NAME=OS VALUE=vsim9>\n";
 ...
 print "<TR><TD>Generate MAC template</TD><TD>$generatemac</TD></TR>\n";
...
sub vsim9_WriteTemplatePXEMenu
{
...
sub vsim9_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  #print "<LI>$destfile\n";
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"vsim9");
  return ($result);
}


sub vsim9_CopyTemplateFile2
{
  local($template)=shift;
  local($destfile)=shift;
...
sub vsim9_GetDefaultCommandLine
{
  local($template)=@_;
...
sub vsim9_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub vsim9_GetDefaultConfigFile2
{
  local($template)=@_;
...
sub vsim9_GetDefaultPublishFile
{
  local($template)=@_;
...
sub vsim9_GetDefaultPublishDir
{
  local($template)=@_;
  local($publishdir)=$TFTPDIR."/pxelinux.cfg/templates/$template";
  return $publishdir;
...
sub vsim9_CreateTemplate
{
  ...
  $config{KERNEL}=&vsim9_GetDefaultKernel($template);
  $config{CMDLINE}=&vsim9_GetDefaultCommandLine($template);
  $config{CONFIGFILE1}=&vsim9_GetDefaultConfigFile1($template);
  $config{CONFIGFILE2}=&vsim9_GetDefaultConfigFile2($template);
  $config{PUBLISHDIR1}=&vsim9_GetDefaultPublishDir($template);
  $config{PUBLISHFILE1}=&vsim9_GetDefaultPublishFile($template);

  #$config{CMDLINE}=$cmdline;
  #$config{KERNEL}=$kernel;

  local($result)=&vsim9_CopyTemplateFile($template,$config{CONFIGFILE1});
  if ($result) 
  { 
  ...
  local($result)=&vsim9_CopyTemplateFile2($template,$config{CONFIGFILE2},$config{OS});
  #if ($result != 0 ) { return $result};
  if ($result) 
  { 
  ...
  local($result)=&{"vsim9_WriteTemplatePXEMenu"}($template,%config);
  if ($result) 
  {
...
sub vsim9_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;

  local($orgfile)=$info{CONFIGFILE2};
  local($destfile)=&vsim9_GetDefaultConfigFile2($desttemplate);
  ...
  local($result)=&kickstart_CopyTemplate("vsim9",$template,$desttemplate,%info);

  return $result;
}


sub vsim9_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"vsim9");
  return $result;
}

sub vsim9_ConfigureTemplate
{
  local($template,%config)=@_;
  # local($result)=&kickstart_ConfigureTemplate("vsim9",$template,%config);
...
sub vsim9_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("vsim9",$template,%info);
...
sub vsim9_NewOS_2
{
 ...
 print "<BR><BR>\n";
 print "<TABLE>\n";
 print "<TR><TD>OS</TD><TD>vsim9</TD></TR>\n";
 print "<TR><TD>Flavor</TD><TD>$formdata{OSFLAVOR}</TD></TR>\n";
 print "</TABLE>\n";
 print "</FORM>\n";
 print "</CENTER>\n";
...
sub vsim9_ImportOS
{
  local($result)=&kickstart_ImportOS("vsim9");
  return $result;
}

sub vsim9_ImportOS_DoIt
{
...
sub vsim9_PublishTemplate
{
    ...
    if ($#indexes<0)
    {
       ...
       local($dstfile)="$subtemplateextractdir/etc/setup.ngsh";
       open(SFILE,"<$srcfile") || &PrintError("Could not open source file $srcfile");
       open(DFILE,">$dstfile") || &PrintError("Could not open target file $dstfile");
       while (<SFILE>)
       {
