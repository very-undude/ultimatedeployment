...
for $curtemplateconfig (sort(keys(%templatesortorder)))
{
  if (defined($templateconfig{$templatesortorder{$curtemplateconfig}}))
  {
    ...
    {
      &Log("Adding sorted template $templatesortorder{$curtemplateconfig} to menu list");
      push(@efitemplates,$templatesortorder{$curtemplateconfig});
      delete($templateconfig{$templatesortorder{$curtemplateconfig}});
    } else {
      &Log("Not adding sorted template $templatesortorder{$curtemplateconfig} to menu list");
    }
  }
}
...
for $mytemplateconfig (sort(keys(%templateconfig)))
{
  ...
  {
    &Log("Adding unsorted template $mytemplateconfig to menu list");
    push(@efitemplates,$mytemplateconfig);
    local(%info)=&GetTemplateInfo($template);
  } else {
    &Log("Not adding unsorted template $mytemplateconfig to menu list");
  }
}
...
{
  &Log("Writing out $efitemplate as the menu item");
...
for $efitemplate (@efitemplates)
{
  &Log("Writing out $efitemplate as the menu action");
  print ":$efitemplate\n";
