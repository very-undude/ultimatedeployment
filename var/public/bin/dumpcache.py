...
if __name__ == '__main__':
    ...
    if mymode == "short":
      ...
      wi2k3num=0
      winxpnum=0
      print '<TR><TD>Total devices</TD><TD>%d</TD></TR>' % len(devlist)
      for dev in devlist.keys():
        if (devlist[dev]['WI2KS'] != 'UNKNOWN'):
           wi2ksnum=wi2ksnum+1
        if (devlist[dev]['WI2K3'] != 'UNKNOWN'):
           wi2k3num=wi2k3num+1
        if (devlist[dev]['WINXP'] != 'UNKNOWN'):
           winxpnum=winxpnum+1
      print "<TR><TD>Windows 2000 Server drivers</TD><TD>%d</TD></TR>" % wi2ksnum
      print "<TR><TD>Windows 2003 Server drivers</TD><TD>%d</TD></TR>" % wi2k3num
      print "<TR><TD>Windows XP Drivers</TD><TD>%d</TD></TR>" % winxpnum
    else:
      print '<TABLE BORDER=1>';
      print '<TR><TD>Device</TD><TD>description</TD><TD>char</TD><TD>svc</TD><TD>Bus Type</TD><TD>inf</TD><TD>WI2KS</TD><TD>WINXP</TD><TD>WI2K3</TD></TR>';
      for dev in devlist.keys():
        print '<TR>';
        print '<TD>%s</TD>' % dev;
        ...
        print '<TD>%s</TD>' % devlist[dev]['WI2KS'];
        print '<TD>%s</TD>' % devlist[dev]['WINXP'];
        print '<TD>%s</TD>' % devlist[dev]['WI2K3'];
        print '</TR>';
      print '</TABLE>';
