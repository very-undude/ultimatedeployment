...
sub debian9_NewTemplate_2
{
 my($result)=&kickstart_NewTemplate_Finish("debian9");
 return $result;
}

sub debian9_WriteTemplatePXEMenu
{
...
sub debian9_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"debian9");
  return ($result);
}

sub debian9_GetDefaultCommandLine
{
  local($template)=@_;
...
sub debian9_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub debian9_GetDefaultPublishFile
{
  local($template)=@_;
...
sub debian9_GetDefaultPublishDir
{
  local($template)=@_;
...
sub debian9_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("debian9");
  return $result;
}

sub debian9_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("debian9",$template,$desttemplate,%info);
  return $result;
}

sub debian9_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"debian9");
  return $result;
}

sub debian9_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("debian9",$template,%config);
  return $result;
}

sub debian9_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("debian9",$template,%info);
  return $result;
}

sub debian9_NewOS_2
{
 local($kickstartos)="debian9";

 local($osflavor)=$formdata{OSFLAVOR};
 print "<CENTER>\n";
 print "<H2>New Operating System Wizard Step 2</H2>\n";
 print "<FORM NAME=WIZARDFORM ENCTYPE=\"multipart/form-data\" METHOD=\"POST\" ACTION=\"/cgi-bin/upload_debian9.cgi\">\n";
...
sub debian9_ImportOS
{
  local($result)=&kickstart_ImportOS("debian9");
  return 0;
}

sub debian9_ImportOS_DoIt
{
  ...
  local($netboot)=$TFTPDIR."/debian9/".$args{OSFLAVOR}."/netboot.tar.gz";
  local($netboot2)=$TFTPDIR."/debian9/".$args{OSFLAVOR}."/netboot2.tar.gz";
  local($initrd)=$TFTPDIR."/initrd.debian9.".$args{OSFLAVOR};
  local($netbootinitrd)=$TFTPDIR."/debian9/".$args{OSFLAVOR}."/debian-installer/$currentarch/initrd.gz";
  ...
  local($command)="tar -C $TFTPDIR/debian9/".$args{OSFLAVOR}." -xvzf $netboot ./debian-installer/$currentarch/initrd.gz";
  ...
  my($command)="mkdir $TFTPDIR/debian9/$args{OSFLAVOR}/niclibs ; cd $TFTPDIR/debian9/$args{OSFLAVOR}/niclibs ; ar -vx $modulesdeb";
  ...
  my($command)="mkdir $TFTPDIR/debian9/$args{OSFLAVOR}/nicmods ; tar -C $TFTPDIR/debian9/$args{OSFLAVOR}/nicmods -xf $TFTPDIR/debian9/$args{OSFLAVOR}/niclibs/data.tar.xz";
  ...
  my($command)="cp -r $TFTPDIR/debian9/$args{OSFLAVOR}/nicmods/lib $netboot.dir ";
