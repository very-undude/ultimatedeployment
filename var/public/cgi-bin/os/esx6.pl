...
sub esx6_NewTemplate_2
{
  local($result)=&kickstart_NewTemplate_Finish("esx6");
  return $result;
}

sub esx6_NewTemplate_Finish
{
 local($result)=&kickstart_NewTemplate_Finish("esx6");
 return $result;
}

sub esx6_WriteTemplatePXEMenu
{
...
sub esx6_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"esx6");
  return ($result);
}

sub esx6_GetDefaultCommandLine
{
  local($template)=@_;
...
sub esx6_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub esx6_GetDefaultPublishFile
{
  local($template)=@_;
...
sub esx6_GetDefaultPublishDir2
{
...
sub esx6_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("esx6");
  return $result;
}

sub esx6_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("esx6",$template,$desttemplate,%info);
  return $result;
}

sub esx6_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"esx6");
  return $result;
}

sub esx6_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("esx6",$template,%config);
  return $result;
}

sub esx6_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("esx6",$template,%info);
  return $result;
}

sub esx6_NewOS_2
{
 local($result)=&kickstart_NewOS_2("esx6");
 return result;
}

sub esx6_ExtraConfiguration
{
  local(%info)=();
...
sub esx6_ImportOS
{
 local($result)=&kickstart_ImportOS("esx6");
  return $result;
}

sub esx6_ImportOS_DoIt
{
  require "general.pl";
  require "config.pl";
  require "action.pl";
  ...
  #local($result)=&kickstart_ImportOS_DoIt("esx6",$kernellocation,$initrdlocation,$actionid,@otherfiles);
  return 0;
}
