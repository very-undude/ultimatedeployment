...
sub esx7_NewTemplate_2
{
  local($result)=&kickstart_NewTemplate_Finish("esx7");
  return $result;
}

sub esx7_NewTemplate_Finish
{
 local($result)=&kickstart_NewTemplate_Finish("esx7");
 return $result;
}

sub esx7_WriteTemplatePXEMenu
{
...
sub esx7_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"esx7");
  return ($result);
}

sub esx7_GetDefaultCommandLine
{
  local($template)=@_;
...
sub esx7_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub esx7_GetDefaultPublishFile
{
  local($template)=@_;
...
sub esx7_GetDefaultPublishDir2
{
...
sub esx7_GetDefaultEFIPublishLink1
{
  local($publishdir)="$WWWDIR/ipxe/templates/[TEMPLATE]/files";
  return $publishdir;
}

sub esx7_GetDefaultKernel
{
  local($template)=@_;
...
sub esx7_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("esx7");
  return $result;
}

sub esx7_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("esx7",$template,$desttemplate,%info);
  return $result;
}

sub esx7_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"esx7");
  return $result;
}

sub esx7_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("esx7",$template,%config);
  return $result;
}

sub esx7_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("esx7",$template,%info);
  return $result;
}

sub esx7_NewOS_2
{
 local($result)=&kickstart_NewOS_2("esx7");
 return result;
}

sub esx7_ExtraConfiguration
{
  local(%info)=();
...
sub esx7_ImportOS
{
 local($result)=&kickstart_ImportOS("esx7");
  return $result;
}

sub esx7_ImportOS_DoIt
{
  require "general.pl";
  require "config.pl";
  require "action.pl";
