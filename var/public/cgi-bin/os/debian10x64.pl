...
sub debian10x64_NewTemplate_2
{
 my($result)=&kickstart_NewTemplate_Finish("debian10x64");
 return $result;
}

sub debian10x64_WriteTemplatePXEMenu
{
...
sub debian10x64_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"debian10x64");
  return ($result);
}

sub debian10x64_GetDefaultCommandLine
{
  local($template)=@_;
...
sub debian10x64_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub debian10x64_GetDefaultPublishFile
{
  local($template)=@_;
...
sub debian10x64_GetDefaultPublishDir
{
  local($template)=@_;
...
sub debian10x64_GetDefaultKernel
{
  local($template)=@_;
...
sub debian10x64_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("debian10x64");
  return $result;
}

sub debian10x64_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("debian10x64",$template,$desttemplate,%info);
  return $result;
}

sub debian10x64_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"debian10x64");
  return $result;
}

sub debian10x64_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("debian10x64",$template,%config);
  return $result;
}

sub debian10x64_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("debian10x64",$template,%info);
  return $result;
}

sub debian10x64_NewOS_2
{
 local($kickstartos)="debian10x64";

 local($osflavor)=$formdata{OSFLAVOR};
 print "<CENTER>\n";
 print "<H2>New Operating System Wizard Step 2</H2>\n";
 print "<FORM NAME=WIZARDFORM ENCTYPE=\"multipart/form-data\" METHOD=\"POST\" ACTION=\"/cgi-bin/upload_debian10x64.cgi\">\n";
...
sub debian10x64_ImportOS
{
  local($result)=&kickstart_ImportOS("debian10x64");
  return 0;
}

sub debian10x64_ImportOS_DoIt
{
  ...
  local($netboot)=$TFTPDIR."/debian10x64/".$args{OSFLAVOR}."/netboot.tar.gz";
  local($netboot2)=$TFTPDIR."/debian10x64/".$args{OSFLAVOR}."/netboot2.tar.gz";
  local($initrd)=$TFTPDIR."/initrd.debian10x64.".$args{OSFLAVOR};
  local($netbootinitrd)=$TFTPDIR."/debian10x64/".$args{OSFLAVOR}."/debian-installer/$currentarch/initrd.gz";
  ...
  local($command)="tar -C $TFTPDIR/debian10x64/".$args{OSFLAVOR}." -xvzf $netboot ./debian-installer/$currentarch/initrd.gz";
  ...
  my($command)="mkdir $TFTPDIR/debian10x64/$args{OSFLAVOR}/niclibs ; cd $TFTPDIR/debian10x64/$args{OSFLAVOR}/niclibs ; ar -vx $modulesdeb";
  ...
  my($command)="mkdir $TFTPDIR/debian10x64/$args{OSFLAVOR}/nicmods ; tar -C $TFTPDIR/debian10x64/$args{OSFLAVOR}/nicmods -xf $TFTPDIR/debian10x64/$args{OSFLAVOR}/niclibs/data.tar.xz";
  ...
  my($command)="cp -r $TFTPDIR/debian10x64/$args{OSFLAVOR}/nicmods/lib $netboot.dir ";
