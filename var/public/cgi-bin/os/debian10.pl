...
sub debian10_NewTemplate_2
{
 my($result)=&kickstart_NewTemplate_Finish("debian10");
 return $result;
}

sub debian10_WriteTemplatePXEMenu
{
...
sub debian10_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"debian10");
  return ($result);
}

sub debian10_GetDefaultCommandLine
{
  local($template)=@_;
...
sub debian10_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub debian10_GetDefaultPublishFile
{
  local($template)=@_;
...
sub debian10_GetDefaultPublishDir
{
  local($template)=@_;
...
sub debian10_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("debian10");
  return $result;
}

sub debian10_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("debian10",$template,$desttemplate,%info);
  return $result;
}

sub debian10_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"debian10");
  return $result;
}

sub debian10_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("debian10",$template,%config);
  return $result;
}

sub debian10_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("debian10",$template,%info);
  return $result;
}

sub debian10_NewOS_2
{
 local($kickstartos)="debian10";

 local($osflavor)=$formdata{OSFLAVOR};
 print "<CENTER>\n";
 print "<H2>New Operating System Wizard Step 2</H2>\n";
 print "<FORM NAME=WIZARDFORM ENCTYPE=\"multipart/form-data\" METHOD=\"POST\" ACTION=\"/cgi-bin/upload_debian10.cgi\">\n";
...
sub debian10_ImportOS
{
  local($result)=&kickstart_ImportOS("debian10");
  return 0;
}

sub debian10_ImportOS_DoIt
{
  ...
  local($netboot)=$TFTPDIR."/debian10/".$args{OSFLAVOR}."/netboot.tar.gz";
  local($netboot2)=$TFTPDIR."/debian10/".$args{OSFLAVOR}."/netboot2.tar.gz";
  local($initrd)=$TFTPDIR."/initrd.debian10.".$args{OSFLAVOR};
  local($netbootinitrd)=$TFTPDIR."/debian10/".$args{OSFLAVOR}."/debian-installer/$currentarch/initrd.gz";
  ...
  local($command)="tar -C $TFTPDIR/debian10/".$args{OSFLAVOR}." -xvzf $netboot ./debian-installer/$currentarch/initrd.gz";
  ...
  my($command)="mkdir $TFTPDIR/debian10/$args{OSFLAVOR}/niclibs ; cd $TFTPDIR/debian10/$args{OSFLAVOR}/niclibs ; ar -vx $modulesdeb";
  ...
  my($command)="mkdir $TFTPDIR/debian10/$args{OSFLAVOR}/nicmods ; tar -C $TFTPDIR/debian10/$args{OSFLAVOR}/nicmods -xf $TFTPDIR/debian10/$args{OSFLAVOR}/niclibs/data.tar.xz";
  ...
  my($command)="cp -r $TFTPDIR/debian10/$args{OSFLAVOR}/nicmods/lib $netboot.dir ";
