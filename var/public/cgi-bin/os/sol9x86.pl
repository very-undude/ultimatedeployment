...
sub sol9x86_NewTemplate_2
{
 local($result)=&kickstart_NewTemplate_Finish("sol9x86");
 return $result;
}

sub sol9x86_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"sol9x86");
  return ($result);
}


sub sol9x86_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub sol9x86_GetDefaultConfigFile2
{
  local($template)=@_;
...
sub sol9x86_GetDefaultPublishFile
{
  local($template)=@_;
...
sub sol9x86_GetDefaultPublishDir
{
  local($template)=@_;
...
sub sol9x86_CreateTemplate
{
  ...
  local($result)=&{"kickstart_CopyTemplateFile"}($template,$config{CONFIGFILE2},"sol9x86_anymachine");
  if ($result != 0 ) { return $result};
...
sub sol9x86_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("sol9x86",$template,$desttemplate,%info);
  return $result;
}

sub sol9x86_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"sol9x86");
  ...
  local($result)=&sol9x86_CreateDhcpIncludeFile();
  if ($result) { return $result; }
...
sub sol9x86_ApplyConfigureTemplate
{
  ...
  local($tempdir)=$TEMPDIR."/sol9x86config.$$";
...
sub sol9x86_PublishTemplate
{
  ...
  if ($#indexes<0)
  {
    ...
    local($result)=&sol9x86_CreateTarFile(%info);
      ...
      for $sub (keys(%subinfo))
      {
       if ($sub ne "__HEADER__")
       {
         ...
         local($result)=&sol9x86_CreateTarFile(%subinfo);
      }
    }
    close(MENUFILE);
  }

  local($result)=&sol9x86_CreateTemplateDhcpConfig(%info);
  if ($result) { return $result; }
...
sub sol9x86_CreateDhcpIncludeFile
{
...
sub sol9x86_CreateTemplateDhcpConfig
{
  local(%info)=@_;
...
sub sol9x86_NewOS_2
{
 local($result)=&kickstart_NewOS_2("sol9x86");
 return result;
}

sub sol9x86_ImportOS
{
 local($result)=&kickstart_ImportOS("sol9x86");
  return $result;
}

sub sol9x86_ImportOS_DoIt
{
  local($actionid)=shift;

  local($kernellocation1)="/Solaris_9/Tools/Boot/usr/platform/i86pc/lib/fs/nfs/inetboot";

  local($kernellocation1a)="/Solaris_9/Tools/Boot/platform/i86pc/inetboot";
  ...
  $osinfo{MOUNTPOINT_1}="/solaris9x86/$osinfo{FLAVOR}";
  $osinfo{FILE_1}="$TFTPDIR/inetboot.i86pc.$osinfo{OS}.$osinfo{FLAVOR}";
  $osinfo{MOUNTONBOOT}=$args{MOUNTONBOOT};
  $osinfo{NFSEXPORT_1}="/solaris9x86/$osinfo{FLAVOR}";
  ...
  { 
    ...
    if ($result) 
    { 
      &UpdateActionProgress($actionid,-2,"Could not copy kernel inetboot for i86pc"); 
      return 5;
    }
  }
  &UpdateActionProgress($actionid,50,"Copied kernel inetboot for i86pc");
