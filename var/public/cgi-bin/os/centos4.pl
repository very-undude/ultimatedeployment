...
sub centos4_NewTemplate_2
{
 local($result)=&kickstart_NewTemplate_Finish("centos4");
 return $result;
}

sub centos4_WriteTemplatePXEMenu
{
...
sub centos4_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"centos4");
  return ($result);
}

sub centos4_GetDefaultCommandLine
{
  local($template)=@_;
...
sub centos4_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub centos4_GetDefaultPublishFile
{
  local($template)=@_;
...
sub centos4_GetDefaultPublishDir
{
  local($template)=@_;
...
sub centos4_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("centos4");
  return $result;
}

sub centos4_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("centos4",$template,$desttemplate,%info);
  return $result;
}

sub centos4_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"centos4");
  return $result;
}

sub centos4_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("centos4",$template,%config);
  return $result;
}

sub centos4_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("centos4",$template,%info);
  return $result;
}
...
sub centos4_NewOS_2
{
 ...
 print "<INPUT TYPE=HIDDEN NAME=OS VALUE=centos4>\n";
 print "<INPUT TYPE=HIDDEN NAME=OSFLAVOR VALUE=\"$formdata{OSFLAVOR}\">\n";
 print "<script language='javascript' src='/js/centos4.js'></script>\n";
...
sub centos4_ImportOS
{
 # local($result)=&kickstart_ImportOS("centos4");
 ...
 print "<INPUT TYPE=HIDDEN NAME=actionid ID=actionid VALUE=\"$actionid\">\n";
  ...
  print "<CENTER>\n";
  print "<TABLE>\n";
  print "<TR><TD>Operating System</TD><TD>centos4</TD></TR>\n";
  ...
  &PrintStatusDiv();
  print "</CENTER>\n";
  &RunAction($actionid,"Import flavor $flavor (centos4)","os/centos4.pl","\&centos4\_ImportOS_DoIt($actionid);");
