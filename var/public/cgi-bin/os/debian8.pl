...
sub debian8_NewTemplate_2
{
 my($result)=&kickstart_NewTemplate_Finish("debian8");
 return $result;
}

sub debian8_WriteTemplatePXEMenu
{
...
sub debian8_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"debian8");
  return ($result);
}

sub debian8_GetDefaultCommandLine
{
  local($template)=@_;
...
sub debian8_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub debian8_GetDefaultPublishFile
{
  local($template)=@_;
...
sub debian8_GetDefaultPublishDir
{
  local($template)=@_;
...
sub debian8_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("debian8");
  return $result;
}

sub debian8_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("debian8",$template,$desttemplate,%info);
  return $result;
}

sub debian8_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"debian8");
  return $result;
}

sub debian8_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("debian8",$template,%config);
  return $result;
}

sub debian8_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("debian8",$template,%info);
  return $result;
}

sub debian8_NewOS_2
{
 local($kickstartos)="debian8";

 local($osflavor)=$formdata{OSFLAVOR};
 print "<CENTER>\n";
 print "<H2>New Operating System Wizard Step 2</H2>\n";
 print "<FORM NAME=WIZARDFORM ENCTYPE=\"multipart/form-data\" METHOD=\"POST\" ACTION=\"/cgi-bin/upload_debian8.cgi\">\n";
...
sub debian8_ImportOS
{
  local($result)=&kickstart_ImportOS("debian8");
  return 0;
}

sub debian8_ImportOS_DoIt
{
  ...
  local($netboot)=$TFTPDIR."/debian8/".$args{OSFLAVOR}."/netboot.tar.gz";
  local($netboot2)=$TFTPDIR."/debian8/".$args{OSFLAVOR}."/netboot2.tar.gz";
  local($initrd)=$TFTPDIR."/initrd.debian8.".$args{OSFLAVOR};
  local($netbootinitrd)=$TFTPDIR."/debian8/".$args{OSFLAVOR}."/debian-installer/$currentarch/initrd.gz";
  ...
  local($command)="tar -C $TFTPDIR/debian8/".$args{OSFLAVOR}." -xvzf $netboot ./debian-installer/$currentarch/initrd.gz";
  ...
  my($command)="mkdir $TFTPDIR/debian8/$args{OSFLAVOR}/niclibs ; cd $TFTPDIR/debian8/$args{OSFLAVOR}/niclibs ; ar -vx $modulesdeb";
  ...
  my($command)="mkdir $TFTPDIR/debian8/$args{OSFLAVOR}/nicmods ; tar -C $TFTPDIR/debian8/$args{OSFLAVOR}/nicmods -xf $TFTPDIR/debian8/$args{OSFLAVOR}/niclibs/data.tar.xz";
  ...
  my($command)="cp -r $TFTPDIR/debian8/$args{OSFLAVOR}/nicmods/lib $netboot.dir ";
