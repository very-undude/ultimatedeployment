...
sub sol9sparc_NewTemplate_2
{
 local($result)=&kickstart_NewTemplate_Finish("sol9sparc");
 return $result;
}

sub sol9sparc_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"sol9sparc");
  return ($result);
}


sub sol9sparc_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub sol9sparc_GetDefaultConfigFile2
{
  local($template)=@_;
...
sub sol9sparc_GetDefaultPublishFile
{
  local($template)=@_;
...
sub sol9sparc_GetDefaultPublishDir
{
  local($template)=@_;
...
sub sol9sparc_CreateTemplate
{
  ...
  local($result)=&{"kickstart_CopyTemplateFile"}($template,$config{CONFIGFILE2},"sol9sparc_anymachine");
  if ($result != 0 ) { return $result};
...
sub sol9sparc_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("sol9sparc",$template,$desttemplate,%info);
  return $result;
}

sub sol9sparc_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"sol9sparc");
  ...
  local($result)=&sol9sparc_CreateDhcpIncludeFile();
  if ($result) { return $result; }
...
sub sol9sparc_ConfigureTemplate
{
...
sub sol9sparc_ApplyConfigureTemplate
{
  ...
  local($tempdir)=$TEMPDIR."/sol9sparcconfig.$$";
...
sub sol9sparc_PublishTemplate
{
  ...
  if ($#indexes<0)
  {
    ...
    local($result)=&sol9sparc_CreateTarFile(%info);
      ...
      for $sub (keys(%subinfo))
      {
       if ($sub ne "__HEADER__")
       {
         ...
         local($result)=&sol9sparc_CreateTarFile(%subinfo);
      }
    }
    close(MENUFILE);
  }

  local($result)=&sol9sparc_CreateTemplateDhcpConfig(%info);
  if ($result) { return $result; }
...
sub sol9sparc_CreateDhcpIncludeFile
{
...
sub sol9sparc_CreateTemplateDhcpConfig
{
  local(%info)=@_;
  ...
  print DHCPCONF "option SUNW.install-path \"/solaris9sparc/$info{FLAVOR}\";\n";
  print DHCPCONF "option SUNW.root-server-hostname \"uda\";\n";
  print DHCPCONF "option SUNW.root-server-ip-address $system{UDA_IPADDR};\n";
  print DHCPCONF "option SUNW.root-path-name \"/solaris9sparc/$info{FLAVOR}/Solaris_9/Tools/Boot\";\n";
  print DHCPCONF "next-server $system{UDA_IPADDR};\n";
  print DHCPCONF "if substring ( option vendor-class-identifier, 0, 10) = \"SUNW.SPARC\" {";
  print DHCPCONF "     filename \"inetboot.sun4m.$info{OS}.$info{FLAVOR}\";\n";
  print DHCPCONF " } else {\n";
...
sub sol9sparc_NewOS_2
{
 local($result)=&kickstart_NewOS_2("sol9sparc");
 return result;
}

sub sol9sparc_ImportOS
{
 local($result)=&kickstart_ImportOS("sol9sparc");
  return $result;
}

sub sol9sparc_ImportOS_DoIt
{
  local($actionid)=shift;

  local($kernellocation1)="/Solaris_9/Tools/Boot/usr/platform/sun4u/lib/fs/nfs/inetboot";
  local($kernellocation2)="/Solaris_9/Tools/Boot/usr/platform/sun4m/lib/fs/nfs/inetboot";

  local($kernellocation1a)="/Solaris_9/Tools/Boot/platform/sun4u/inetboot";
  local($kernellocation2a)="/Solaris_9/Tools/Boot/platform/sun4m/inetboot";
  ...
  $osinfo{MOUNTPOINT_1}="/solaris9sparc/$osinfo{FLAVOR}";
  $osinfo{FILE_1}="$TFTPDIR/inetboot.sun4u.$osinfo{OS}.$osinfo{FLAVOR}";
  $osinfo{FILE_2}="$TFTPDIR/inetboot.sun4m.$osinfo{OS}.$osinfo{FLAVOR}";
  $osinfo{MOUNTONBOOT}=$args{MOUNTONBOOT};
  $osinfo{NFSEXPORT_1}="/solaris9sparc/$osinfo{FLAVOR}";
  ...
  { 
    ...
    if ($result) 
    {
      &UpdateActionProgress($actionid,-2,"Could not copy kernel inetboot for sun4m"); 
      return 5;
    }
  }
  &UpdateActionProgress($actionid,70,"Copied kernel inetboot for sun4m");
