...
sub suse12_NewTemplate_2
{
 local($result)=&kickstart_NewTemplate_Finish("suse12");
 return $result;
}

sub suse12_WriteTemplatePXEMenu
{
...
sub suse12_CopyTemplateFile
{
  local($template)=shift;
  local($destfile)=shift;
  local($result)=&kickstart_CopyTemplateFile($template,$destfile,"suse12");
  return ($result);
}

sub suse12_GetDefaultCommandLine
{
  local($template)=@_;
...
sub suse12_GetDefaultConfigFile1
{
  local($template)=@_;
...
sub suse12_GetDefaultPublishFile
{
  local($template)=@_;
...
sub suse12_GetDefaultPublishDir
{
  local($template)=@_;
...
sub suse12_CreateTemplate
{
  local($result)=&kickstart_CreateTemplate("suse12");
  return $result;
}

sub suse12_CopyTemplate
{
  local($template,$desttemplate,%info)=@_;
  local($result)=&kickstart_CopyTemplate("suse12",$template,$desttemplate,%info);
  return $result;
}

sub suse12_DeleteTemplate
{
  local($template)=shift;
  local($result)=&kickstart_DeleteTemplate($template,"suse12");
  return $result;
}

sub suse12_ConfigureTemplate
{
  local($template,%config)=@_;
  local($result)=&kickstart_ConfigureTemplate("suse12",$template,%config);
  return $result;
}

sub suse12_ApplyConfigureTemplate
{
  local($template,%info)=@_;
  
  local($result)=&kickstart_ApplyConfigureTemplate("suse12",$template,%info);
  return $result;
}

sub suse12_NewOS_2
{
 # local($result)=&kickstart_NewOS_2("suse12");
local($osflavor)=$formdata{OSFLAVOR};
 ...
 print "<INPUT TYPE=HIDDEN NAME=OS VALUE=\"suse12\">\n";
 print "<INPUT TYPE=HIDDEN NAME=OSFLAVOR VALUE=\"$formdata{OSFLAVOR}\">\n";
 print "<script language='javascript' src='/js/suse12.js'></script>\n";
...
sub suse12_ImportOS
{
 local($result)=&kickstart_ImportOS("suse12");
  return $result;
}

sub suse12_ImportOS_DoIt
{
  ...
  local($result)=&kickstart_ImportOS_DoIt("suse12",$kernellocation,$initrdlocation,$actionid);
  return $result;
}
