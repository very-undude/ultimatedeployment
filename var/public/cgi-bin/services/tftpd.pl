...
sub tftpd_ConfigureService
{
  local($service)="tftpd";
  local($configfilename)="/etc/tftpd.conf";
...
sub tftpd_ApplyConfigureService
{
  local($service)="tftpd";
  local($configfilename)="/etc/tftpd.conf";
  ...
  {
   local($command)="/usr/bin/systemctl enable $service";
   ...
   local($command)="/usr/bin/systemctl disable $service";
  ...
  local($pid)=&GetServicePID($service);
   if ($pid =~ /^[0-9]+$/)
   {
     ...
     local($command)="/usr/bin/systemctl stop $service";
     ...
     local($command)="/usr/bin/systemctl start $service";
