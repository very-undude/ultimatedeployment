...
$osinfo{NETBOOTIMAGE}="debian10x64/$osinfo{FLAVOR}/".$query->param('NETBOOTIMAGE');

local($osdir)="/var/public/tftproot/debian10x64";
...
require "os/debian10x64.pl";
%formdata=();
$formdata{OS}="debian10x64";
...
local($result)=&debian10x64_ImportOS("debian10x64");
if ($result)
{
  &PrintError("Could Import OS debian 9 with flavor $osinfo{FLAVOR}");
  exit 4;
}

# &PrintSuccess("Import of debian10x64 operating system $osinfo{FLAVOR} succesfull");
